...
class CSGOCdn extends EventEmitter {
   ...
   */
  cleanup() {
    const files = readdirSync(`./${this.config.directory}`).filter((fn) =>
      fn.startsWith(this.config.fileList),
    );
  ...
  getLatestManifestId() {
    ...
    return this.getProductInfo().then(
      ([apps, packages, unknownApps, unknownPackages]) => {
  ...
  /**
   * Parses the items_game.txt and the csgo_english.txt file
   *
   * will also invert the csgo_english.txt file to make it easier to search for the correct item
   *
   */
  loadResources() {
    this.itemsGame = vdf.parse(
      this.vpkDir.getFile("scripts/items/items_game.txt").toString(),
    )["items_game"];
    this.csgoEnglish = vdf.parse(
      this.vpkDir.getFile("resource/csgo_english.txt").toString(),
    )["lang"]["Tokens"];

    this.weaponNameMap = Object.keys(this.csgoEnglish).filter((n) =>
      n.startsWith("SFUI_WPNHUD"),
    );
  ...
  async downloadFiles() {
    return new Promise((resolve, reject) => {
      exec(
        `./${this.config.directory}/${this.config.depotDownloader} -app ${APP_ID} -depot ${DEPOT_ID} -filelist ${this.config.directory}/${this.config.fileList}-${pid} -dir ${this.config.directory} -os windows -osarch 64 max-downloads 16 -max-servers 40 -validate`,
        (error, stdout) => {
  ...
  getPathURL(path, type = "vpk") {
    ...
    if (type === "vpk") {
      ...
      this.log.debug(
        `Reading local file ${this.config.directory + "/" + path}...`,
      );
      // check if file exists
      if (!existsSync(`./${this.config.directory}/${path}`)) {
        this.log.error(
          `Failed to retrieve ${path} in VPK, do you have the package category enabled in options?`,
        );
        return;
      }
  ...
  getStickerURL(name, large = true) {
    ...
    const path = this.vpkStickerFiles.find((t) =>
      t.endsWith(`${fileName}.vtex_c`),
    );
  ...
  getPatchURL(name, large = true) {
    ...
    const fileName = large ? `${name}_large_png` : `${name}_png`;
    const path = this.vpkPatchFiles.find((t) =>
      t.endsWith(`${fileName}.vtex_c`),
    );
  ...
  getStatusIconURL(name, size = "") {
    ...
    const path = this.vpkStatusIconFiles.find((t) =>
      t.endsWith(`${fileName}.vtex_c`),
    );
  ...
  getWeaponTest(name, size = "") {
    if (!this.ready) {
      return;
    }

    const fileName = size ? `${name}_${size}_png` : `${name}_png`;
    const path = this.weaponsFiles.find((t) =>
      t.endsWith(`${fileName}.vtex_c`),
    );
  ...
  getWeaponURL(defindex, paintindex) {
    ...
    console.log(weaponName, skinName);
  }
  ...
  getStickerNameURL(marketHashName) {
    ...
    for (const tag of this.csgoEnglish["inverted"][stickerName] || []) {
      ...
      const url = this.getStickerURL(
        stickerKits[kitIndex].sticker_material,
        true,
      );
  ...
  getGraffitiNameURL(marketHashName, large = true) {
    ...
    for (const tag of this.csgoEnglish["inverted"][graffitiName] || []) {
      ...
      kitIndices.sort((a, b) => {
        const index1 =
          !!stickerKits[a].name && stickerKits[a].name.indexOf("graffiti");
        const index2 =
          !!stickerKits[b].name && stickerKits[b].name.indexOf("graffiti");
        if (index1 === index2) {
          return 0;
        } else if (index1 > -1) {
          return -1;
    ...
    for (const t of weaponTags) {
      ...
      for (const key of this.csgoEnglish["inverted"][skinName] || []) {
        ...
        for (const paintindex of paintindexes) {
          ...
          const path = (
            paintKit ? `${weaponClass}_${paintKit}` : weaponClass
          ).toLowerCase();
        }
      }
    }
  ...
  getItemNameURL(marketHashName, phase) {
    ...
    console.log("strippedMarketHashName", strippedMarketHashName);
